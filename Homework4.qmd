...
NOTE: for the intercept you dont need to worry having it since it wasnt listed just but an empty beta 0/ 

Write the equation of a first-order model for hotel image ($y$) as a function of the six ambient conditions.

$$
\hat{y} = (\text{unknown intercept})\beta_0 + + 0.122(\text{Air Quality}) + 0.018(\text{Temperature}) + \\
0.124(\text{Odor/Aroma}) + 0.119(\text{Music}) +
0.101(\text{Noise/Sound Level}) + 0.463(\text{Overall Image}) + \varepsilon 
$$
...
-   ($\beta_{0}$ = 0.463): The intercept represents the predicted value of ($\hat{y}$) (hotel image) when all six ambient condition variables are equal to zero.
