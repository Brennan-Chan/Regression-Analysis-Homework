...
# only use when initally viewing the data 
...
The coefficient of determination is ($R^2 = 0.7768$), and the adjusted ($R^2 = 0.752$). This indicates that approximately 77.68% of the variability in Earnings is explained by the linear relationship with Events and Score, and after adjusting for the number of predictors, about \$75.2% of the variation is still explained.
...
## Part B:
### Which of the independent variables, Events or Score, would you choose if you want
only one predictor?
```{r}
mod_spec = set_engine(linear_reg(), "lm")
...
adjR2_events = g_events$adj.r.squared
adjR2_score = g_score$adj.r.squared
AIC_events = g_events$AIC
AIC_score = g_score$AIC

print(c(adjR2_events = adjR2_events, adjR2_score = adjR2_score))
print(c(AIC_events = AIC_events, AIC_score = AIC_score))

```
The model using Score has a substantially larger adjusted $R^2$ (0.6729 vs 0.3045) and a smaller AIC (646.42 vs 662.22). Therefore, if restricted to one predictor, we retain Score as the single predictor of Earnings.
