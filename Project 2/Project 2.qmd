...
we can see from the residuals vs leverage plot that there are 13 points (106, 6, 83, 158, 28, 71, 130, 147, 80, 110, 112, 133, 37) that stand out as potentially influential outliers. To further investigate these points, we can look at the Cook's distance bar plot and DFBETAS plot.
...
okay now that we have found the extreme outliers lets see if the model improves when removed and refitted 
```{r}
extreme_outliers <- c(6, 130, 147, 83, 71, 8, 106, 28, 158, 110)

clean_train <- train_data[-extreme_points, ]
...
cleaned_summary
```
the model showed clear improvement in overall fit. The adjusted $R^2$ increased from $0.86384$ to $0.8883$, and the residual standard error ($\sigma$) decreased substantially (from $2971.6$ to $2173.0$), indicating a tighter fit around the regression line. In addition, both AIC and BIC were noticeably lower for the cleaned model, and the deviance was reduced by more than half, suggesting a considerably better representation of the underlying data.

These results indicate that the influential observations were inflating the error structure and degrading model performance. Because the model without these points provides a better statistical fit and more stable parameter estimates, we proceed with the cleaned model for the remaining diagnostic checks and the final validation step.
